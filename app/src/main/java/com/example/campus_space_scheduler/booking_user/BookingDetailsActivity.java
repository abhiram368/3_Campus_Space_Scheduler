...
public class BookingDetailsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // If data is missing (common with notifications), fetch it from Firebase
        if (bookingId != null && (spaceName == null || spaceName.isEmpty() || spaceName.equalsIgnoreCase("N/A"))) {
            Log.d(TAG, "Triggering full booking fetch for ID: " + bookingId);
            fetchFullBookingDetails(bookingId);
        }
    ...
    private void fetchFullBookingDetails(String id) {
        DatabaseReference bookingRef = FirebaseDatabase.getInstance().getReference("bookings").child(id);
        bookingRef.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.exists()) {
                    ...
                    if (remarks == null) {
                        remarks = snapshot.child("remark").getValue(String.class);
                    }
                    String actionBy = snapshot.child("actionBy").getValue(String.class);
                    String bookedById = snapshot.child("bookedBy").getValue(String.class);
                    String approvedByUid = snapshot.child("approvedBy").getValue(String.class);
                    String lorUrl = snapshot.child("lorUpload").getValue(String.class);
                    boolean hasLor = (lorUrl != null && !lorUrl.isEmpty());

                    // Requested Time
                    String requestedDate = snapshot.child("bookedTime").child("date").getValue(String.class);
                    String requestedTime = snapshot.child("bookedTime").child("time").getValue(String.class);
                    String requestedOn = (requestedDate != null ? requestedDate : "") + " " + (requestedTime != null ? requestedTime : "");

                    // Update UI components
                    updateUI(spaceName, status, date, timeSlot, purpose, description, requestedOn, bookedById, approvedByUid, remarks, actionBy, hasLor, lorUrl);
                }
            }
    ...
    private void updateUI(String sn, String st, String dt, String ts, String prp, String dsc, String ro, String bbi, String abu, String rm, String ab, boolean hl, String lu) {
        TextView spaceNameTextView = findViewById(R.id.textViewSpaceName);
        TextView statusTextView = findViewById(R.id.textViewStatus);
        TextView dateTimeTextView = findViewById(R.id.textViewDateTime);
        TextView purposeTextView = findViewById(R.id.textViewPurpose);
        ...
        if (spaceNameTextView != null) spaceNameTextView.setText(sn != null ? sn : "N/A");
        if (statusTextView != null) {
            statusTextView.setText(st != null ? st.toUpperCase() : "PENDING");
            updateStatusUI(st, statusTextView);
        }
        if (dateTimeTextView != null) dateTimeTextView.setText((dt != null ? dt : "") + " | " + (ts != null ? ts : ""));
        if (purposeTextView != null) purposeTextView.setText(prp != null ? prp : "N/A");
        if (descriptionTextView != null) descriptionTextView.setText(dsc != null ? dsc : "No description provided.");
        if (requestedOnTextView != null) requestedOnTextView.setText(ro != null ? ro : "N/A");

        if (bbi != null && bookedByTextView != null) {
            fetchUserName(bbi, bookedByTextView, null);
        }

        if (st != null) {
            boolean isApproved = st.equalsIgnoreCase("Approved") || st.equalsIgnoreCase("Accepted");
            if (isApproved && buttonAddToCalendar != null) {
                buttonAddToCalendar.setVisibility(View.VISIBLE);
                buttonAddToCalendar.setOnClickListener(v -> checkCalendarPermissions());
            }

            if (approvedByTextView != null && !st.equalsIgnoreCase("Pending")) {
                String label = st.equalsIgnoreCase("Rejected") ? "Rejected by: " : "Approved by: ";
                if (abu != null && !abu.isEmpty()) {
                    fetchUserName(abu, approvedByTextView, label);
                } else if (ab != null && !ab.isEmpty()) {
                    approvedByTextView.setText(label + ab);
                    approvedByTextView.setVisibility(View.VISIBLE);
                }
            }
        }

        if (textViewRemarks != null && rm != null && !rm.trim().isEmpty()) {
            textViewRemarks.setText("Remarks: " + rm);
            textViewRemarks.setVisibility(View.VISIBLE);
        }

        if (buttonViewLor != null && hl && lu != null && !lu.isEmpty()) {
            buttonViewLor.setVisibility(View.VISIBLE);
            buttonViewLor.setOnClickListener(v -> {
                try {
                    Intent browserIntent = new Intent(Intent.ACTION_VIEW, Uri.parse(lu));
                    startActivity(browserIntent);
                } catch (Exception e) {}
            });
        }
